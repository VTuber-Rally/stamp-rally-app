...
export function SuccessPage() {
  ...
  const { currentParticipation, isPending } = useFollowParticipation();
  ...
  useEffect(() => {
    if (currentParticipation?.drawnAt && currentParticipation.isWinner) {
      if (!confettiLaunched) {
...
const StatusBlock = ({
      ...
      <details className="mt-4">
        ...
        <p>
          {t("contest.success.winner.additionalInformation", {
            ...
            drawnAt: participation.drawnAt
              ? new Date(participation.drawnAt).toLocaleDateString()
              : "euh..",
  ...
  // if drawnAt is defined, it means the draw has happened
